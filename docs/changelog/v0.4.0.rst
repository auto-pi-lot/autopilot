...
v0.4.0 - A Turn for the Multifarious (July 30, 2021)
------------------------------------------------------
...
* `Registries & Plugins <https://github.com/wehr-lab/autopilot/pull/109>`_ - Autopilot now supports users writing their code
  ...
  is the basic method, with a few aliases for specific object types like ``autopilot.get_hardware()``.
...
* New Transforms
    * timeseries - adapted a Kalman filter from the wonderful filterpy package! it's in the new timeseries transform module
    * geometry - IMU_Orientation performs a sensor fusion algorithm with the Kalman Filter class to combine gyroscope and accelerometer measurements into a better estimate of earth-centric roll and pitch.
    ...
    * timeseries - Filter_IIR implements scipy's IIR filter as a transform object.
    * timeseries - Integrate adds successive numbers together (scaled by dt if requested). not much by itself, but when used with a kalman filter very useful :)
...
* Terminal can be opened maximized, or have its size and position set explicitly, preserve between launches (ty Chris) - https://github.com/wehr-lab/autopilot/pull/70
...
* fix durations in solenoid class (ty Chris) - https://github.com/wehr-lab/autopilot/pull/63
